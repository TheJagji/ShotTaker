...
STATUS_DIR   = data_path("status")
COMMANDS_DIR = data_path("commands")
STATIC_DIR   = data_path("static")          # writable (icons, uploaded sounds)
SOUNDS_DIR   = data_path("static", "sounds")
...
# =========================
def ensure_dirs():
    for d in [DATA_DIR, STATUS_DIR, COMMANDS_DIR, STATIC_DIR, SOUNDS_DIR]:
...
def seed_static():
    ...
    for root, _, files in os.walk(src):
        ...
        if rel_root.split(os.sep)[0] == "sounds":
            continue
        for fn in files:
