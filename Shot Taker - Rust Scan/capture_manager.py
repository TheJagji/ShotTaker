...
def next_counter(game_name, shot_type):
    with _counter_lock:
        ...
        key = f"{game_name.casefold()}_{shot_type}"
...
def reset_counters(game_name=None):
    with _counter_lock:
        if game_name is None:
            ...
            counters = load_counters()
            pref = game_name.casefold() + "_"
            for k in [k for k in counters if k.casefold().startswith(pref)]:
                del counters[k]
            save_counters(counters)
...
def capture_screenshot(game_name, shot_type, settings, reason="", gate=True):
    ...
    Uses rust_capture (DXGI) if available, otherwise mss.
    """
    if not AVAILABLE:
        print("[CAP] capture unavailable (mss/PIL/numpy missing and rust_capture not built)")
        return None
    try:
        monitor_setting = settings.get("monitor", "auto")
        if frames.RUST_AVAILABLE:
            # Rust handles the grab internally — no mss context needed
            img = frames.grab(None, monitor_setting)
