...
def last_capture():
    return jsonify(config.read_json(os.path.join(config.STATUS_DIR, "last_capture.json"), None))
...
def upload_sound():
    ...
    config.update_settings(notification_sound=f"/user_data/sounds/notification{ext}")
    return jsonify({"success": True, "path": f"/user_data/sounds/notification{ext}"})
...
def delete_sound():
    ...
    if old.startswith("/user_data/sounds/"):
        full = os.path.join(config.SOUNDS_DIR, os.path.basename(old))
...
@app.route("/user_data/sounds/<path:filename>")
def serve_user_sound(filename):
    return send_from_directory(config.SOUNDS_DIR, filename)
...
@app.route("/api/cloud/copy", methods=["POST"])
def cloud_copy():
    """Copy selected screenshots to the configured cloud folder."""
    data = request.json or {}
    paths = data.get("paths", [])
    s = config.load_settings()
    cloud_folder = s.get("cloud_folder", "").strip()
    if not cloud_folder:
        return jsonify({"success": False, "error": "No cloud folder configured"})
    if not os.path.isdir(cloud_folder):
        return jsonify({"success": False, "error": f"Cloud folder not found: {cloud_folder}"})
    copied = 0
    errors = []
    for p in paths:
        if not os.path.exists(p):
            errors.append(f"Not found: {os.path.basename(p)}")
            continue
        try:
            import shutil
            dest = os.path.join(cloud_folder, os.path.basename(p))
            ...
            errors.append(str(e))
    if errors:
        return jsonify({"success": copied > 0, "copied": copied, "error": "; ".join(errors)})
    return jsonify({"success": True, "copied": copied})


@app.route("/api/game/icon/<path:exe>")
def game_icon(exe):
    """Serve Steam game icon from local cache if available."""
    try:
        import glob
        # Look in Steam's icon cache
        cache_dirs = []
        s = config.load_settings()
        # Try to find Steam from configured folders
        folders = config.read_json(config.FOLDERS_FILE, {}) or {}
        for folder in folders.get("steam", []):
            # Walk up to find Steam root
            parts = folder.replace(chr(92), "/").split("/")
            for i in range(len(parts), 0, -1):
                candidate = "/".join(parts[:i])
                icon_dir = os.path.join(candidate, "steam", "games")
                if os.path.isdir(icon_dir):
                    cache_dirs.append(icon_dir)
                icon_dir2 = os.path.join(candidate, "appcache", "librarycache")
                if os.path.isdir(icon_dir2):
                    cache_dirs.append(icon_dir2)

        # Also try default Steam paths
        for d in [
            os.path.expandvars(r"%PROGRAMFILES(X86)%\Steam\steam\games"),
            os.path.expandvars(r"%PROGRAMFILES(X86)%\Steamppcache\librarycache"),
        ...
        for d in cache_dirs:
            # Look for any icon file matching the exe name
            for ext in ("*.ico", "*.png", "*.jpg"):
                matches = glob.glob(os.path.join(d, ext))
                for m in matches:
                    if exe_lower in os.path.basename(m).lower():
                        return send_from_directory(os.path.dirname(m), os.path.basename(m))
