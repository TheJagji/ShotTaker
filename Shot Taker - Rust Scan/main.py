...
def command_loop():
    ...
    focus_flag = os.path.join(config.COMMANDS_DIR, "focus.flag")
    while True:
        try:
            ...
            if os.path.exists(focus_flag) and os.path.getsize(focus_flag) > 0:
                open(focus_flag, "w").close()
                print("[CMD] Focus requested — bringing window to front.")
                try:
                    window_manager.show()
                except Exception as e:
                    print(f"[CMD] Focus error: {e}")
        except Exception as e:
